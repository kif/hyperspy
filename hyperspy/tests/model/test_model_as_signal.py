...
class TestModelAsSignal:
    ...
    def test_model_as_signal1(self):
        m = self.model
        g1 = self.g1
        g2 = self.g2
        s_m = m.as_signal()
        g1_data = g1.function(m.axes_manager.signal_axes[0].axis) 
        g2_data = g2.function(m.axes_manager.signal_axes[0].axis) 
        assert_true(np.all(s_m[0][0] == (g1_data+g2_data)))

    def test_model_as_signal2(self):
        m = self.model
        g1 = self.g1
        s_m = m.as_signal(component_list=[g1])
        g1_data = g1.function(m.axes_manager.signal_axes[0].axis) 
        assert_true(np.all(s_m[0][0] == g1_data))
